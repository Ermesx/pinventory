...
Scope: Definitions only; for behavior, rules, and constraints see doc/pins-invariants.md.

### Glossary
- Address: Postal address; value object used on a Pin.
- GooglePlaceId: Identifier for a Google Place; value object.
- Import: An import job for a user and source (e.g., Google Data Portability); see doc/pins-invariants.md for workflow
  rules.
...
- Import creates new Pins and updates existing Pins from external source data to keep the dataset in sync with the
  user’s starred places.
...
See doc/pins-invariants.md for the authoritative list of business rules, constraints, and invariants.
